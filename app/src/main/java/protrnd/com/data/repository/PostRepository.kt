...
class PostRepository(private val api: ProfileApi, private val postsApi: PostApi) : BaseRepository() {
    ...
    suspend fun getProfilesByUsername(name: String) = safeApiCall { api.getProfilesByUsername(name) }

    suspend fun addImageToFirebase(uris: List<Uri>, username: String, fileType: List<String>): List<String> {
        ...
        for (position in uris.indices) {
            try {
                ...
                val downloadUrl = fileReference.putFile(uris[position]).await().storage.downloadUrl.await()
